...
err_exit:
	...
	mysql_mutex_init(0, &ibuf_mutex, nullptr);
	mysql_mutex_init(0, &ibuf_pessimistic_insert_mutex, nullptr);
	...
	/* TODO: Merge the change buffer at startup! */
...
/************************************************************************//**
Reset the free bits of the page in the ibuf bitmap. This is done in a separate
...
void ibuf_reset_free_bits(buf_block_t *block)
{
      ...
      ibuf_bitmap_get_map_page(id, block->zip_size(), &mtr))
  {
    ...
    ibuf_bitmap_page_set_bits<IBUF_BITMAP_FREE>
      (bitmap_page, id, block->physical_size(), 0, &mtr);
  }
	...
	mtr_t*		mtr)	/*!< in: mtr */
{
  ...
  ibuf_reset_free_bits_low(*block1, mtr);
  ibuf_reset_free_bits_low(*block2, mtr);
}
